...
export async function run(argv: string[]): Promise<void> {
  ...
  if (command === 'init') {
    ...
    const path = writeClientConfig(process.cwd(), flags.client);
...
export function parseInitArgv(args: string[]): { client: McpClient } {
  ...
  if (!client) {
    throw new Error(
      `Usage: ng-elemental-mcp init --client <${MCP_CLIENTS.join('|')}>`,
    );
  }
  ...
  return { client: client as McpClient };
}
...
Usage:
  ...
  npx @ng-elemental/mcp init --client cursor|claude|vscode|codex
