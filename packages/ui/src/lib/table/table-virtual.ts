export interface ElTableVirtualWindow {
  start: number;
  end: number;
  padStart: number;
  padEnd: number;
...
export function tableVirtualWindow(options: {
  scrollTop: number;
  viewportHeight: number;
  itemCount: number;
  itemHeight: number;
  overscan?: number;
}): ElTableVirtualWindow {
...
export function tableCellText(row: object, name: string): string {
